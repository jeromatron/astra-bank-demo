...
const AccountInfoBox = () => {
  ...
  const [account_id,setAccountNo] = useState(null)
  const [name,setName] = useState(null)
  const [phoneNumber,setphoneNumber] = useState(null)
  
  const fetchData = async () => {
    const account = window.location.search
    const results = await axios.get(`/.netlify/functions/getAccountData${account_id}`)
  ...
  return (
    <div className="AccountItem">
        ...
        <p className="featuredAccountTitle">A/C # : <span className="accountSubtitle">{account_id}</span></p>
